...
let data = await fetch(this.url);
  ...
  this.setState({ article:parse_data.hits, starting_count:parse_data.from, ending_count:parse_data.to, next_page:parse_data._links.next.href})
...
let data = await fetch(this.state.Home_page);
  let parse_data= await data.json();
  this.setState({ article:parse_data.hits, starting_count:1, ending_count:parse_data.to, next_page:parse_data._links.next.href})
...
async componentDidMount(){
  ...
  let parse_data= await data.json();
  this.setState({ article:parse_data.hits, starting_count:parse_data.from, ending_count:parse_data.to, next_page:parse_data._links.next.href, Home_page:url})
}
  ...
  return <div className="col-md-6 mt-4" key ={element.recipe.uri}>
   ...
   <RecipeItem title={element.recipe.label} ImagesUrl={element.recipe.images.SMALL.url} url={element.recipe.shareAs}
    health_labels={
      (element.recipe.healthLabels.toString()).substring(0,145)
    }
     Ingridiants={((element.recipe.ingredientLines.toString()).replace(","," and ")).substring(0,110)} 
     caleroies={Math.ceil(element.recipe.calories)} 
     fat={element.recipe.totalNutrients.FAT.quantity?Math.ceil(element.recipe.totalNutrients.FAT.quantity):" "}
     ...
     vitamin_a={element.recipe.totalNutrients.VITA_RAE.quantity}
  ...
  <div className="d-flex justify-content-between my-3">
        <button type="button" className="btn btn-dark" onClick={this.home_handler} disabled={this.state.starting_count<=1} >Home</button>
        <button type="button" className="btn btn-dark" onClick={this.next_handler} >Next</button>
        </div>  
</div>
